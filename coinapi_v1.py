...
class HTTPClient:
    def __init__(self, endpoint, headers=dict(), params=dict(), use_proxy=False):
    ...
    def perform(self):
        ...
        if (self.use_proxy):
            pr = ProxyRequests(resource)
            pr.set_headers(self.headers)
            pr.get()
            raw_response = pr.request
            print(pr.proxy_used)
        else:
...
class CoinAPIv1:
    ...
    def __init__(self, api_key, headers=dict(), client_class=HTTPClient, use_proxy=False):
    ...
    def metadata_list_exchanges(self):
        ...
        client = self.client_class(request.endpoint(), self.headers, use_proxy=self.use_proxy)
    ...
    def exchange_rates_get_specific_rate(self,
        ...
        client = self.client_class(request.endpoint(),
                                   self.headers,
                                   request.query_parameters, use_proxy=self.use_proxy)
        return client.perform()
